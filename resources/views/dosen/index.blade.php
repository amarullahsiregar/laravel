        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-lg-flex">
                        <div>
                            <h1 class="text-4xl font-bold">Dosen Aktif Membimbing Tugas Akhir</h1>
                        </div>
                        <div class="ms-auto my-auto mt-lg-0 mt-4">
                            <div class="ms-auto my-auto">
                                <a href="{{'/dosen-add'.'/'.$key}}" class="bg-gradient-to-r from-cyan-500 to-blue-500 border-2 border-solid border-gray-500 hover:bg-blue-400 text-white py-2 px-4 rounded">
                                    <button type="submit" class="btn  btn-md mt-4 mb-4 ">+&nbsp; Tambah dosen</button>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
                <table class="table table-flush dataTable-table m-5 border-2 " id="users-list">
                    <thead class="thead-light">
                        <tr class="">
                            <th data-sortable="" class="border border-t-2 w-1/12"><a href="#" class="dataTable-sorter">No Induk</a></th>
                            <th data-sortable="" class="border border-t-2 w-1/6"><a href="#" class="dataTable-sorter">Nama Dosen</a></th>
                            <th data-sortable="" class="border border-t-2 w-1/6"><a href="#" class="dataTable-sorter">Email</a></th>
                            <th data-sortable="" style="width:4%" class="border border-t-2 xl:w-1/12"><a href="#" class="dataTable-sorter">Inisial</a></th>
                            <th data-sortable="" style="width:4%" class="border border-t-2 w-1/12"><a href="#" class="">Edit</a></th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($dosens as $dosen)
                        <tr>
                            <td class="text-sm pl-5 border text-left">{{ $dosen->id }}</td>
                            <td class="text-sm pl-5 border font-bold">{{ $dosen->nama }}</td>
                            <td class="text-sm pl-5 border">{{ $dosen->email }}</td>
                            <td class="text-sm pl-5 border">{{ $dosen->inisial_dosen }}</td>
                            <td class="text-sm px-16 py-1 border">
                                <a href="/dosen/{{ $dosen->email }}" class="my-3" data-bs-toggle="tooltip" data-bs-original-title="Edit user">
                                    <span>
